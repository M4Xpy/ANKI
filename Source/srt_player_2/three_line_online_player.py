...
def time_delay(pause, start_time, subtitle):
    ...
    if pause:
        time.sleep(0.2)
        keyboard.send("space")
    start_time = time.time() + len(subtitle) * 0.05
    return start_time
...
def split_text_with_punctuation(text, punctuation='].,;:!?)'):
    ...
    # """
    parts = re.split(f"([{punctuation}])", text.replace(" mr.", " mr").replace("Mr.", "Mr").replace(" sir.", " sir").replace("Sir.", "Sir").replace("*", ""))
    parts.append('')
    result = []
    compares = []
    severe = False
...
def no_repit(text, test=False):
    ...
    for part in start:
        ...
        if part_strip_lower not in past_subtitles:
            ...
            for word in part_strip_lower_split:
                word = "".join(letter for letter in word.strip(' -') if letter.isalpha() or letter in "'-").removesuffix("'s")
                if word not in top_300 and word :
                    len_part_strip_lower_split = len(part_strip_lower_split)
                    ratio = past_subtitles.count(word) / len_part_strip_lower_split + top_2000.count(word)
                    if ratio < 0.2 or ratio == 1: #  len_part_strip_lower_split > 7 or
    ...
    stat_txt(f"('{text}', ' xxxxxx', '{show}', '{play}', {compare})")
